...
(check-expect (find Text "part4") #false)
(check-expect (find Libs "read!") (list "Docs"))
(check-expect (find TS "hang") (list "Libs" "Code"))
(define (find d f-target)
  (local
    ; Processes a directory
    ; Dir -> [Maybe Path]
    ((define (handle-dir d)
       (cond [(in-files? (dir-files d)) '()]
             [else (handle-dirs (dir-dirs d))]))
