...
(check-expect (find-all Text "part3") `((,"part3")))
(check-expect (find-all Libs "read!") `((,"Code" ,"Docs" ,"read!") (,"Docs" ,"read!")))
(check-expect (find-all TS "read!") `((,"read!") (,"Libs" ,"Code" ,"Docs" ,"read!") (,"Libs" ,"Docs" ,"read!")))
(define (find-all d target)
  (local
    ...
    ((define (handle-dir d)
     ...
     (define filtered-result
       (filter (lambda (path)
              (ormap (lambda (address)
                       (string=? address target))
                     path))
              (handle-dir d))))
    (if (empty? filtered-result)
        #false
        filtered-result)))